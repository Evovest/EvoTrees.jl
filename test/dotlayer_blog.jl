...
p1 = plot(X_train[:,1], Y_train, ms = 3, zcolor=Y_train, color=cgrad(["darkred", "#33ccff"]), msw=0, background_color = "white", seriestype=:scatter, xaxis = ("var2"), yaxis = ("target"), leg = false, cbar=true)
...
plot!(fill(μ, params1.nbins), lw=3, color="#99ee99", background_color = "white", seriestype=:line, label="predict", leg=true)
# savefig(p1, "var1.svg")
...
savefig(p, "first_split.svg")
savefig(p, "first_split.png")
...
savefig(p, "left_parabole.svg")
savefig(p, "left_parabole.png")
loss.(Ref(left_res), 0.0) - loss.(Ref(left_res), mean(left_res))
...
pred = zeros(SVector{params1.K,Float64}, size(X_train,1)) .+ μ
...
savefig(p, "tree_group.png")
savefig(p1, "tree_1.png")
