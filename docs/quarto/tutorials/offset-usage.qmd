...
Feature importance from the offset model shows that the importance of `Sex` has significantly shrinked, while `Age` has maintained a strong importance.
The residual EvoTree model captures the non-linear and interaction effect missed by the GLM, as well as effect from the additional features. 
For `Sex`, since it's a binary feature, much of its signal was already captured by the GLM.
