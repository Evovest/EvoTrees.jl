...
function _pearson_chunk!(scores, weights, p, y, w, group, chunk, K::Int, stride::Int)
    pred = Float64[]
    obs = Float64[]
    wt = Float64[]
    for g in chunk
        ...
        scored = 0
        for k in 1:K
            s = _pearson_group!(pred, obs, wt, p, y, w, rows, k, stride * (k - 1) + 1)
...
function pearson(
    ...
    weights = zeros(Float64, ng)
    @threads for chunk in _group_chunks(ng)
        _pearson_chunk!(scores, weights, p, y, w, group, chunk, K, stride)
    end
    sw = sum(weights)
