...
"""
    train(params::EvoTypes{L,T}, dtrain::AbstractDataFrame;
    target_name,
    fnames_num=nothing,
    fnames_cat=nothing,
    w_name=nothing,
...
- `dtrain::DataFrames`: configuration info providing hyper-paramters. `EvoTypes` comprises EvoTreeRegressor, EvoTreeClassifier, EvoTreeCount and EvoTreeMLE.
...
function train(
    ...
    # initialize model and cache
    if String(params.device) == "gpu"
        m, cache = init_gpu(params, x_train, y_train; target_name, fnames_num, fnames_cat, w_name, offset_name, group_name)
    else
        m, cache = init(params, x_train, y_train; target_name, fnames_num, fnames_cat, w_name, offset_name, group_name)
    end
