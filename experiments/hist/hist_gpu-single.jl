...
nidx_cpu = rand(15:31, nobs)

∇_gpu = CuArray(∇)
x_bin_gpu = CuArray(x_bin)
# dimensions: [2 * K + 1, nbins, feats, nb_nodes]
h∇_cpu = zeros(Float64, 3, nbins, nfeats, 2^(max_depth - 1) - 1);
...
# laptop - 1M: 1.063 ms (127 allocations: 9.05 KiB)
@btime update_nodes_idx_gpu!(nidx_src, nidx_gpu, is_gpu, x_bin_gpu, cond_feats_gpu, cond_bins_gpu, feattypes_gpu)

#####################################
# update gains
#####################################
function update_gains_kernel!(nidx, is, x_bin, cond_feats, cond_bins, feattypes)
    tix = threadIdx().x
    bdx = blockDim().x
    bix = blockIdx().x
    gdx = gridDim().x
...
function update_gains_gpu!(h∇L_gpu, h∇R_gpu, h∇_gpu)
    cumsum!(h∇L_gpu, h∇_gpu; dims=2)
    h∇R_gpu .-= h∇L_gpu .= view(h∇_gpu, :, 1:1, :, :)
...
CUDA.@time update_gains_gpu!(h∇L_gpu, h∇R_gpu, h∇_gpu)

# laptop - 1M:  42.700 μs (168 allocations: 9.81 KiB)
@btime update_gains_gpu!(h∇L_gpu, h∇R_gpu, h∇_gpu)
