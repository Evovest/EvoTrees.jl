...
for _device in device_list
        ...
        for nfeats in nfeats_list
            for max_depth in max_depth_list
                for group_size in group_size_list
                    ...
                    group_train = UInt32.((0:nobs-1) .÷ group_size .+ 1)
                    ...
                    if nobs == first(nobs_list) && nfeats == first(nfeats_list) && max_depth == first(max_depth_list)
                        @info "warmup"
                        _m_evo = EvoTrees.fit(params_evo; x_train, y_train, group_train, x_eval=x_train, y_eval=y_train, group_eval=group_train, print_every_n=100)
                    ...
                    t_train_evo = @elapsed m_evo = EvoTrees.fit(params_evo; x_train, y_train, group_train, x_eval=x_train, y_eval=y_train, group_eval=group_train, print_every_n=100)
    ...
    select!(df, Cols(:device, :nobs, :nfeats, :max_depth, :group_size, r"train_", r"infer_"))
    path = joinpath(@__DIR__, "results", "regressor-grps-$loss-$_device.csv")
    CSV.write(path, df)
end
